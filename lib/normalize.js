...
// CC-CEDICT's own leading labels decide whether a sense is a standalone
// translation. Register and era marks flag marginal usages ("(coll.)
// what?" on 吗, "(classical particle …)" on 乎); restriction shapes flag
// context-bound meanings ("(of a time etc) last" on 去). Domain and
// category labels ("(meteorology)", "(specifier)") restrict nothing —
// "(meteorology) climate" IS the word for climate.
const REGISTER_LABEL = /\((?:coll|sl\b|slang|dialect|vulgar|Tw\b|HK\b|Cantonese|Beijing|Internet|old\b|archaic|literary|classical|humorous|derog|onom|loanword|euphemism|courteous|honorific|humble|deferential|respectful|polite|dated|obsolete)/i;
const RESTRICTION_LABEL = /\((?:bound form|abbr|of |used |usu\.|when |before |after |followed |preceded |in |as |more\)|one's|sb's|attached|placed)/i;
// Trailing marks that make a sense compound-bound or register-bound even
// though the gloss text leads: "China (abbr.)" on 中, "your (honorific)"
// on 高. Narrow on purpose — "(informal, as opposed to …)" must not match.
const TRAILING_MARK = /\((?:abbr|honorific|humble|deferential|polite|courteous|modest)\b[^)]*\)$/i;
const LEADING_QUALIFIERS = /^(?:\([^)]*\)\s*)+/;
...
export function exactGlossRank(entry, key) {
  ...
  for (const definition of entry.d) {
    for (const sense of definition.split(';')) {
      if (normalizeSense(sense) === key) return position;
      position++;
    }
  }
  return Infinity;
...
/**
 * Does [entry] carry [key] as a TRANSLATABLE sense — one a page word may be
 * swapped for? Returns which case variants match: `base` for a lowercase
 * gloss ("march" on 游行), `proper` for a capitalized one ("March" on 三月).
 * CC-CEDICT capitalization is deliberate (周日 is "Sunday"), so it is the
 * signal that separates months and proper nouns from their homographs.
 *
 * A sense is translatable unless its qualifier marks a register/era or a
 * usage restriction. A bare sense INHERITS the qualifier of the run it
 * sits in — 乎's "than" lives under "(classical particle …)", which scopes
 * until the next parenthetical, per the dictionary's own convention.
 */
export function translatableGloss(entry, key) {
  let inherited = '';
  let position = 0;
  const match = { base: Infinity, proper: Infinity };
  for (const definition of entry.d) {
    for (const sense of definition.split(';')) {
      const trimmed = sense.trim();
      const lead = trimmed.match(LEADING_QUALIFIERS);
      const qualifier = lead ? lead[0] : inherited;
      if (lead) inherited = lead[0];
      const at = position++;
      if (normalizeSense(trimmed) !== key) continue;
      if (REGISTER_LABEL.test(qualifier) || RESTRICTION_LABEL.test(qualifier)) continue;
      if (TRAILING_MARK.test(trimmed)) continue;
      const bare = trimmed.replace(LEADING_QUALIFIERS, '');
      if (/^surname /.test(bare)) continue;
      if (/^[A-Z]/.test(bare)) match.proper = Math.min(match.proper, at);
      else match.base = Math.min(match.base, at);
    }
  }
  return match;
}
...
export function lookup(text, dictionary) {
  ...
  for (const variant of variations) {
    // hasOwn, not indexing: "constructor" is a real English word, and the
    // inherited prototype value made this throw.
    const ids = Object.hasOwn(dictionary.index, variant) ? dictionary.index[variant] : undefined;
