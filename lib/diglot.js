/**
 * Diglot weave replacement — English words the user has a known Chinese
 * word for, swapped in place on the page.
 *
...
import { normalizeWord, exactGlossRank, translatableGloss } from './normalize.js';
import { WEAVE_REFUSALS } from './weave_refusals.js';

// Bump on ANY change to buildReplacementMap, the known-word derivation,
// the gloss-label rules in normalize.js, or the generated refusal data:
...
// (The stored stamp also carries DICT_HASH, so dictionary rebuilds deploy
// without a bump.)
export const MAP_VERSION = 8;

// Refusals grouped by key once at module load — the per-candidate string
// concat of a flat set measured ~45ms per map build.
const REFUSALS_BY_KEY = new Map();
for (const pair of WEAVE_REFUSALS) {
  // Last space: keys may contain spaces ("able to bear"), hanzi never does.
  const split = pair.lastIndexOf(' ');
  const key = pair.slice(0, split);
  const hanzi = pair.slice(split + 1);
  const set = REFUSALS_BY_KEY.get(key);
  if (set) set.add(hanzi);
  else REFUSALS_BY_KEY.set(key, new Set([hanzi]));
}
...
export function buildReplacementMap(knownHanzi, dictionary) {
  ...
  for (const [key, ids] of Object.entries(dictionary.index)) {
    ...
    const refused = REFUSALS_BY_KEY.get(key);
    for (const id of ids) {
      const entry = dictionary.entries[id];
      if (!entry) continue;
      // Index order sorts exact-sense candidates ahead of mere mentions,
      // so the first candidate with NO exact sense ends the walk — 59% of
      // candidates are mentions and skipping them was ~40% of build time.
      // (Do not break on translatableGloss's infinities: a qualified sense
      // still counts as exact here, and later candidates can still win.)
      if (exactGlossRank(entry, key) === Infinity) break;
      // Audit-refused pairs are skipped, not dropped: the canonical falls
      // to the next positional candidate (her: 其 refused -> 她).
      if (refused !== undefined && refused.has(entry.s)) continue;
    ...
    // A proper canonical the user has NOT learned must not let capitalized
    // tokens fall back to the base sense — "March 2024" would weave 游行.
    // The marker keeps capitalized tokens English until it is learned.
    if (map[key] !== undefined && map[key].proper === undefined
        && proper !== null && proper !== base) {
      map[key].contested = true;
    }
  }
  return map;
}
  ...
  while (i < tokens.length) {
    ...
    for (let len = Math.min(maxPhraseTokens, tokens.length - i); len >= 1 && !hit; len--) {
      ...
      variants?.find((v) => {
        ...
        else if (capitalized && value.contested === true) return false;
