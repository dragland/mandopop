...
// Same copy as the Android panel — the two clients should fail in the
// same words.
function friendlySignInError(body) {
  if (body.includes('INVALID_LOGIN_CREDENTIALS') || body.includes('INVALID_PASSWORD')) {
    return 'Traverse rejected that email or password';
  }
  if (body.includes('EMAIL_NOT_FOUND')) return 'No Traverse account for that email';
  if (body.includes('TOO_MANY_ATTEMPTS')) return 'Too many attempts — wait and try again';
  if (body.includes('MFA') || body.includes('SECOND_FACTOR')) {
    return 'This account uses two-factor auth, which mandopop does not support yet';
  }
  return null;
}
...
export async function signIn(email, password) {
  ...
  if (!response.ok) {
    throw new TraverseError(
      friendlySignInError(body) ?? `Traverse sign-in failed: HTTP ${response.status} ${body.slice(0, 400)}`,
      response.status,
    );
  }
...
async function refresh(refreshToken) {
  ...
  // The account can change while the request is in flight (sign-out, or
  // sign-out plus sign-in as someone else). Every outcome below must only
  // touch storage if it still belongs to the session that started this
  // refresh — otherwise a stale refresh signs out the new account or
  // plants the old account's rotated token under it.
  const auth = await readLocal(AUTH_KEY);
  const sameSession = auth?.refreshToken === refreshToken;

  if (response.status === 400) {
    // The refresh token is dead — a retry can never succeed.
    if (sameSession) await signOut();
  ...
  if (!sameSession) throw new TraverseError('Not signed in to Traverse');
  ...
  if (json.refresh_token && json.refresh_token !== refreshToken) {
    updates[AUTH_KEY] = { ...auth, refreshToken: json.refresh_token };
  }
