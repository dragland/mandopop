...
// The stored map is stale when EITHER the rules or the dictionary change —
// a dict:build without a MAP_VERSION bump must still deploy.
const MAP_STAMP = `${MAP_VERSION}:${DICT_HASH}`;
...
async function syncInternal(dictionary, force) {
  ...
  try {
    ...
    let state = stored[SYNC_STATE_KEY] ?? {};
    let rows = stored[SCHEDULES_KEY] ?? [];
    let content = stored[CARDS_KEY] ?? {};

    // Account boundary: a session-expiry sign-out deliberately keeps the
    // mirror (refilling costs ~940 billed reads), so a DIFFERENT account
    // signing in later must not inherit it. Same account resumes cheaply.
    if (state.accountUid !== undefined && state.accountUid !== acct.uid) {
      await chrome.storage.local.remove([
        SCHEDULES_KEY, CARDS_KEY, KNOWN_WORDS_KEY, REPLACEMENT_MAP_KEY, MAP_VERSION_KEY,
      ]);
      state = {};
      rows = [];
      content = {};
      stored[KNOWN_WORDS_KEY] = [];
      stored[REPLACEMENT_MAP_KEY] = undefined;
      stored[MAP_VERSION_KEY] = undefined;
    }
    state.accountUid = acct.uid;

    await guardedSet(acct.uid, { [SYNC_STATE_KEY]: { ...state, syncing: true, progressAtMs: Date.now() } });
    ...
    if (backfill.changed || pull || knownWords.length === 0 || stored[MAP_VERSION_KEY] !== MAP_STAMP) {
      ...
      const unchanged = stored[MAP_VERSION_KEY] === MAP_STAMP
    ...
    // Session expiry signs the account out DURING this sync — the account
    // guard would silently swallow exactly the error the user most needs
    // to see, so it is bypassed for that one case.
    const expired = error.statusCode === 400 && /session expired/i.test(error.message ?? '');
    const still = await account();
    if (expired || still?.uid === acct.uid) await recordError(message);
