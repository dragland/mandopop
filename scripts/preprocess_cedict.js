...
const processDict = () => {
  ...
  for (const line of lines) {
    ...
    // Normalize to NFC so tone-marked vowels are single code points,
    // matching the precomposed forms used downstream (JSON + Android SQLite).
  ...
  // Emit a content hash so the service worker can key its IndexedDB cache on
  // the dictionary's contents rather than the extension version. Regenerating
  // the dictionary then invalidates stale caches automatically, with no
  // manifest version bump required. Android invalidates via its own sha256.
  const hash = crypto.createHash('sha256').update(json).digest('hex').slice(0, 16);
  const versionPath = new URL('../dict_version.js', import.meta.url);
  fs.writeFileSync(
    versionPath,
